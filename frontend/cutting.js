...
function renderCuttingQueueContent(sheet) {
  ...
  card.innerHTML =
    ...
    '<div class="muted"><strong>Sheet ' + sheet.sheetPosition + ' of ' + sheet.totalSheetsInOrder + '</strong></div>' +
    '<div class="muted">Target: ' + sheet.cuttingTimeTarget + ' min' + (sheet.includesSetup ? ' (includes machine setup)' : '') + ' · This sheet yields: ' + sheet.parts.join(', ') + '</div>';
